...
export default function PaymentPage() {
  ...
  useEffect(() => {
    // Redirect to shipping if cart is empty or required data is missing
    if (items.length === 0 || !checkoutData.shippingAddress || !checkoutData.shippingMethod) {
      router.push('/checkout/shipping');
      return;
    }
    setCurrentStep('payment');
  }, [items.length, checkoutData.shippingAddress, checkoutData.shippingMethod, router]);
    ...
    <main className="relative min-h-screen bg-background overflow-hidden flex flex-col">
      ...
      <div className="flex-1 pt-32 pb-24">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 space-y-8">
          ...
          <PaymentStep onNext={handleNext} onPrevious={handlePrevious} />
        </div>
      </div>
